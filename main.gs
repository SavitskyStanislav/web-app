...
class SpreadsheetService {
  static getParticipationData() {
    try {
      // Открываем таблицу по ID и получаем конкретный лист по имени
      const sheet = SpreadsheetApp.openById(CONFIG.SHEET_ID).getSheetByName('ru');
  ...
  static getParticipationDataEn() {
    try {
      // Открываем таблицу по ID и получаем конкретный лист по имени
      const sheet = SpreadsheetApp.openById(CONFIG.SHEET_ID).getSheetByName('en');
...
function getParticipationDataEn() {
  return SpreadsheetService.getParticipationDataEn();
}
